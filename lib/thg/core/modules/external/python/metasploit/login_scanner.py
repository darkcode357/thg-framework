...
def run_scanner(args, login_callback):
    ...
    if isinstance(userpass, str) or (
        "unicode" in vars(__builtins__) and isinstance(userpass, unicode)
    ):
    ...
    for [username, password] in userpass:
        try:
            ...
            if login_callback(rhost, rport, username, password):
                module.log(
                    "{}:{} - [{:>{pad_to}}/{}] - {}:{} - Success".format(
                        rhost, rport, curr, total, username, password, pad_to=pad_to
                    ),
                    level="good",
                )
            ...
            module.log(
                "{}:{} - [{:>{pad_to}}/{}] - {}:{} - Error: {}".format(
                    rhost, rport, curr, total, username, password, e, pad_to=pad_to
                ),
                level="error",
            )
