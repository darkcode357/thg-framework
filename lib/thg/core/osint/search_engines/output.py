...
def create_json_data(search_engines):
    ...
    jobj = {
        ...
        u"results": {
            se.__class__.__name__: [i for i in se.results] for se in search_engines
        },
    }
...
def create_html_data(search_engines):
    ...
    for engine in search_engines:
        ...
        for i, v in enumerate(engine.results, 1):
            ...
            link = (
                _replace_with_bold(query, v["link"])
                if u"url" in engine._filters
                else v["link"]
            )
            rows += HtmlTemplate.row.format(
                number=i, href=v["link"], link=link, data=data
            )
...
Level = namedtuple("Level", ["info", "warning", "error"])(
    info=u"INFO ", warning=u"WARNING ", error=u"ERROR "
)
