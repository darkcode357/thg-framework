...
def main(argv=None):
    ...
    console.add_argument(
        ...
        nargs="?",
        const="1",
        help='Ask before exiting thg or accept "exit -y"',
    )
    console.add_argument(
        "-H",
        "--history-file FILE",
    ...
    console.add_argument(
        "-O", "--output FILE", action="store_true", help="Output to the specified file"
    )
    console.add_argument(
        "-p", "--plugin PLUGIN", action="store_true", help=" Load a plugin on startup"
    )
    console.add_argument(
    ...
    DataBase.add_argument(
        "-n", "--no-database", nargs="?", const="1", help="Disable database support"
    )
    DataBase.add_argument(
        "-y",
        "--yaml",
    ...
    framework_op.add_argument(
        "-c File", nargs="?", const="1", help="Load the specified configuration file"
    )

    module_op = parser.add_argument_group("Module options")
    module_op.add_argument(
        "--defer-module-loads",
        nargs="?",
        const="1",
    ...
    module_op.add_argument(
        ...
        nargs="?",
        const="1",
        help="Load an additional module path",
    )
    ...
    launchGroup.add_argument(
        "--rest", action="store_true", help="Run thg and the RESTful API."
    )
    launchGroup.add_argument(
        "--headless",
        action="store_true",
    ...
    restGroup.add_argument(
        "--restport", type=int, nargs=1, help="Port to run the thg RESTful API on."
    )
    restGroup.add_argument(
        "--username",
        nargs=1,
